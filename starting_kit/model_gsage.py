...
class Model:
    def __init__(self, model_dir="./", feature_count=7, hidden_channels=256, num_layers=5):
        ...
        self.net = GraphSAGENet(feature_count, hidden_channels, num_layers, dropout).to(self.device)
    ...
    def add_mean_neighbor_degree(self, data):
        ...
        neigh_sum = torch.zeros(n_nodes, dtype=torch.float, device=deg.device)
        neigh_cnt = torch.zeros(n_nodes, dtype=torch.float, device=deg.device)
        ...
        max_neigh_deg = torch.full((n_nodes,), -1e9, dtype=torch.float, device=deg.device)
    ...
    def add_triangle_count_feature(self, data):
        ...
        triangles = nx.triangles(graph_nx)
        n_nodes = data.num_nodes
        tri_feat = torch.tensor([triangles[i] for i in range(n_nodes)], dtype=torch.float).view(-1, 1)
        x = data.x.float()
        if x.dim() == 1:
            x = x.view(-1, 1).float()
    ...
    def add_core_number_feature(self, data):
        ...
        core_dict = nx.core_number(graph_nx)
        n_nodes = data.num_nodes
        core_feat = torch.tensor([core_dict[i] for i in range(n_nodes)], dtype=torch.float).view(-1, 1).to(data.x.device)
    ...
    def recursive_basic_mis(self, data):
        n_nodes = data.num_nodes
        global_labels = torch.full((n_nodes,), -1, dtype=torch.long, device=self.device)

        curr_x = data.x.float().to(self.device)
        curr_edge_index = data.edge_index.to(self.device)
        curr_mapping = torch.arange(n_nodes, device=self.device)
    ...
    def tree_search_mis(self, data, time_budget=5.0, M=16, max_queue_size=256):
        ...
        queue = [(
            ...
            torch.full((n_nodes,), -1, dtype=torch.long, device=self.device),
        )]
